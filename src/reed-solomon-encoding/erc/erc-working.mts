/* --------------------------------------------------------------
   Direct 1‑to‑1 conversion of src/js
   -------------------------------------------------------------- */

export class ReedSolomon {
  public readonly codec: Codec;
  constructor(nSym) {
    this.nSym = nSym || 10;
    this.codec = new Codec();
  }

  encode(str) {
    var data = Utils.unpack(str);

    var chunkSize = 255 - this.nSym;
    var enc = [];

    for (var i = 0; i < data.length; i += chunkSize) {
      var chunk = data.slice(i, i + chunkSize);
      enc = enc.concat(this.codec.encodeMsg(chunk, this.nSym));
    }

    return enc;
  }

  decode(data) {
    var dec = [];

    for (var i = 0; i < data.length; i += 255) {
      var chunk = data.slice(i, i + 255);
      dec = dec.concat(this.codec.correctMsg(chunk, this.nSym));
    }

    return Utils.pack(dec);
  }
};

/* ------------------------------------------------------------------ */
/* Utils – unchanged from the original file (still a plain object)     */
/* ------------------------------------------------------------------ */
export const Utils = {
  pack: function (bytes) {
    var chars = [];

    for (var i = 0, n = bytes.length; i < n; i++) {
      chars.push(String.fromCharCode(bytes[i]));
    }

    return chars.join('');
  },

  unpack: function (str) {
    var bytes = [];

    for (var i = 0, n = str.length; i < n; i++) {
      bytes.push(str.charCodeAt(i));
    }

    return bytes;
  },

  arrayFill: function (size, value) {
    return Array.apply(null, new Array(size)).map(function () {
      return value;
    });
  },

  sliceStep: function (array, from, to, step) {
    var result = Array.prototype.slice.call(array, from, to);
    var final = [];

    for (var i = result.length - 1; i >= 0; i--) {
      (i % step === 0) && final.push(result[i]);
    }

    final.reverse();
    result = final;

    return result;
  },
};

/* ------------------------------------------------------------------ */
/* GaloisField – converted to a class, init → constructor             */
/* ------------------------------------------------------------------ */
class GaloisField {
  constructor() {
    this.gfExp = Utils.arrayFill(512, 1);
    this.gfLog = Utils.arrayFill(256, 0);

    var x = 1;

    for (var i = 1; i < 255; i++) {
      x <<= 1;
      if (x & 0x100) x ^= 0x11d;
      this.gfExp[i] = x;
      this.gfLog[x] = i;
    }

    for (var i = 255; i < 512; i++) {
      this.gfExp[i] = this.gfExp[i - 255];
    }
  }

  mul(x, y) {
    if (x == 0 || y == 0) return 0;
    return this.gfExp[this.gfLog[x] + this.gfLog[y]];
  }

  div(x, y) {
    if (y == 0) throw 'Division by zero.';
    if (x == 0) return 0;

    return this.gfExp[this.gfLog[x] + 255 - this.gfLog[y]];
  }

  polyScale(p, x) {
    var r = [];
    for (var i = 0; i < p.length; i++) r.push(this.mul(p[i], x));
    return r;
  }

  polyAdd(p, q) {
    var pLen = p.length,
      qLen = q.length,
      maxLen = Math.max(pLen, qLen),
      r = Utils.arrayFill(maxLen, 0),
      rLen = r.length;

    for (var i = 0; i < pLen; i++) r[i + rLen - pLen] = p[i];
    for (var i = 0; i < qLen; i++) r[i + rLen - qLen] ^= q[i];

    return r;
  }

  polyMul(p, q) {
    var r = Utils.arrayFill(p.length + q.length - 1, 0);

    for (var j = 0; j < q.length; j++) {
      for (var i = 0; i < p.length; i++) {
        r[i + j] ^= this.mul(p[i], q[j]);
      }
    }

    return r;
  }

  polyEval(p, x) {
    var y = p[0];
    for (var i = 1; i < p.length; i++) y = this.mul(y, x) ^ p[i];
    return y;
  }
};

/* ------------------------------------------------------------------ */
/* Codec – converted to a class, init → constructor                     */
/* ------------------------------------------------------------------ */
class Codec {
  constructor() {
    this.gf = new GaloisField();
  }

  generatorPoly(nSym) {
    var g = [1];
    for (var i = 0; i < nSym; i++) {
      g = this.gf.polyMul(g, [1, this.gf.gfExp[i]]);
    }
    return g;
  }

  encodeMsg(msgIn, nSym) {
    if (msgIn.length + nSym > 255) throw 'Message too long.';

    var gen = this.generatorPoly(nSym);
    var msgOut = Utils.arrayFill(msgIn.length + nSym, 0);

    for (var i = 0; i < msgIn.length; i++) msgOut[i] = msgIn[i];

    for (var i = 0; i < msgIn.length; i++) {
      var coef = msgOut[i];
      if (coef != 0) {
        for (var j = 0; j < gen.length; j++) {
          msgOut[i + j] ^= this.gf.mul(gen[j], coef);
        }
      }
    }

    for (var i = 0; i < msgIn.length; i++) msgOut[i] = msgIn[i];

    return msgOut;
  }

  calcSyndromes(msg, nSym) {
    var r = [];
    for (var i = 0; i < nSym; i++) r.push(this.gf.polyEval(msg, this.gf.gfExp[i]));
    return r;
  }

  correctErrata(msg, synd, pos) {
    var q = [1];

    for (var i = 0; i < pos.length; i++) {
      var x = this.gf.gfExp[msg.length - 1 - pos[i]];
      q = this.gf.polyMul(q, [x, 1]);
    }

    var p = synd.slice(0, pos.length);
    p.reverse();
    p = this.gf.polyMul(p, q);
    p = p.slice(p.length - pos.length, p.length);
    q = Utils.sliceStep(q, q.length & 1, q.length, 2);

    for (var i = 0; i < pos.length; i++) {
      var x = this.gf.gfExp[pos[i] + 256 - msg.length];
      var y = this.gf.polyEval(p, x);
      var z = this.gf.polyEval(q, this.gf.mul(x, x));
      // msg[pos[i]] ^= this.gf.div(y, this.gf.mul(x, z));
      var magnitude = this.gf.div(y, this.gf.mul(x, z));
      console.log(`errval-old[${pos[i]}] =`, magnitude);
      msg[pos[i]] ^= magnitude;
    }

    return msg;
  }

  rsFindErrors(synd, nMess) {
    var errPoly = [1],
      oldPoly = [1];
    var newPoly;

    for (var i = 0; i < synd.length; i++) {
      oldPoly.push(0);
      var delta = synd[i];

      for (var j = 1; j < errPoly.length; j++) {
        delta ^= this.gf.mul(
          errPoly[errPoly.length - 1 - j],
          synd[i - j]
        );
      }

      if (delta != 0) {
        if (oldPoly.length > errPoly.length) {
          newPoly = this.gf.polyScale(oldPoly, delta);
          oldPoly = this.gf.polyScale(errPoly, this.gf.div(1, delta));
          errPoly = newPoly;
        }
        errPoly = this.gf.polyAdd(
          errPoly,
          this.gf.polyScale(oldPoly, delta)
        );
      }
    }

    var errs = errPoly.length - 1;
    if (errs * 2 > synd.length) throw 'Too many errors to correct';

    var errPos = [];

    for (var i = 0; i < nMess; i++) {
      if (this.gf.polyEval(errPoly, this.gf.gfExp[255 - i]) == 0)
        errPos.push(nMess - 1 - i);
    }

    if (errPos.length != errs) return null;
    return errPos;
  }

  public forneySyndromes(synd, pos, nMess) {
    var fsynd = synd.slice(0);

    for (var i = 0; i < pos.length; i++) {
      var x = this.gf.gfExp[nMess - 1 - pos[i]];

      for (var j = 0; j < fsynd.length - 1; j++) {
        fsynd[j] = this.gf.mul(fsynd[j], x) ^ fsynd[j + 1];
      }

      fsynd.pop();
    }

    return fsynd;
  }

  correctMsg(msgIn, nSym) {
    if (msgIn.length > 255) throw 'Message too long';

    var msgOut = msgIn.slice(0);
    var erasePos = [];

    for (var i = 0; i < msgOut.length; i++) {
      if (msgOut[i] < 0) {
        msgOut[i] = 0;
        erasePos.push(i);
      }
    }

    if (erasePos.length > nSym) throw 'Too many erasures to correct';

    var synd = this.calcSyndromes(msgOut, nSym);

    if (Math.max.apply(null, synd) == 0) {
      console.log('nothing', synd)
      return msgOut.slice(0, msgOut.length - nSym);
    }

    var fsynd = this.forneySyndromes(synd, erasePos, msgOut.length);
    var errPos = this.rsFindErrors(fsynd, msgOut.length);

    if (errPos == null) throw 'Could not locate error';

    console.log('correctErrata', msgOut,
      synd,
      erasePos.concat(errPos))
    msgOut = this.correctErrata(
      msgOut,
      synd,
      erasePos.concat(errPos)
    );

    synd = this.calcSyndromes(msgOut, nSym);

    if (Math.max.apply(null, synd) > 0) throw 'Could not correct message';

    return msgOut.slice(0, -nSym);
  }
};